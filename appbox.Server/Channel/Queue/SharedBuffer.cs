    ...
    public abstract unsafe class SharedBuffer : IDisposable
    {
        ...
        protected SharedBuffer(string name, long bufferSize, bool ownsSharedMemory)
        {
            ...
            if (name == String.Empty || name == null)
                throw new ArgumentException("Cannot be String.Empty or null", nameof(name));
            if (ownsSharedMemory && bufferSize <= 0)
                throw new ArgumentOutOfRangeException(nameof(bufferSize), bufferSize, "Buffer size must be larger than zero when creating a new shared memory buffer.");
#if DEBUG
