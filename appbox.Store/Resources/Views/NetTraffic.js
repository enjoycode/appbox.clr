@Component
export default class NetTraffic extends Vue {
    /** 目标实例IP */
    ...
    chartOptions = {
        title: { text: 'Net Traffic', x: 'center' },
    ...
    refresh() {
        sys.Services.MetricService.GetNetTraffic(this.node, this.start, this.end)
            .then(res => {
                this.chartOptions.series.splice(0)
                var seria1 = { type: 'line', name: 'in', data: res[0], showSymbol: false }
                this.chartOptions.series.push(seria1)
                var seria2 = { type: 'line', name: 'out', data: res[1], showSymbol: false }
