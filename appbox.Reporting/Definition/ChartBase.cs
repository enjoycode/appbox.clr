...
namespace appbox.Reporting.RDL
{
    ...
    internal abstract class ChartBase : IDisposable
    {
        ...
        protected Drawing.Rectangle DrawLegend(Report rpt, Graphics g, bool bMarker, bool bBeforePlotDrawn)
        {
            ...
            try     // no matter what we want to dispose of the graphic resources
            {
                if (s == null)
                {
                    drawFont = new Font(Style.DefaultFontFamily, 10);
                ...
                for (int iCol = 1; iCol <= SeriesCount; iCol++)
                {
                    ...
                    if (c != "") //14052008WRP Cater for empty strings in the legend
                    {
                        ...
                        if ((bMarker || isLine) || (ChartDefn.Type == ChartTypeEnum.Scatter && t == typeof(Drawing.HatchBrush)))
                        ...
                        switch (l.Layout)
                        {
                            case LegendLayoutEnum.Row:
                                ...
                                if (c != "") //14052008WRP to cater for empty strings in the legend
                                {
                                    ...
                                    if ((cm != ChartMarkerEnum.None || this.ChartDefn.Type == ChartTypeEnum.Scatter)
                                        && (t == typeof(HatchBrush))) //GJL 110208 - Don't draw pattern for lines or Bubbles
                                    {
                                        HatchBrush hb = (HatchBrush)GetSeriesBrush(rpt, 1, iCol);
        ...
        protected void DrawPlotAreaStyle(Report rpt, Graphics g, Drawing.Rectangle crect)
        {
        ...
        protected void DrawTitle(Report rpt, Graphics g, Title t, appbox.Drawing.Rectangle rect)
        {
            if (t == null)
                return;
        ...
        protected void DrawCategoryLabel(Report rpt, Graphics g, string t, Style a, Drawing.Rectangle rect)
        {
        ...
        protected bool getNoMarkerVal(Report rpt, int row, int col)
        {
            try
            {
                return bool.Parse(((ChartExpression)_DataDefn[col, row].DisplayItem).NoMarker.Source);
        ...
        protected void DrawDataPoint(Report rpt, Graphics g, Drawing.Rectangle rect, int row, int col)
        {
            DrawDataPoint(rpt, g, Point.Empty, rect, row, col);
        }

        void DrawDataPoint(Report rpt, Graphics g, Point p, Drawing.Rectangle rect, int row, int col)
        {
            MatrixCellEntry mce = _DataDefn[row, col];
            if (mce == null)
                return;                 // Not sure what this really means TODO:
        ...
        protected void AdjustMargins(appbox.Drawing.Rectangle legendRect, Report rpt, Graphics g)
        {
