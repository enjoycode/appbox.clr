...
namespace appbox.Reporting.RDL
{
	...
	internal class Code : ReportLink
	{
		...
		private Assembly GetAssembly()
		{
            ...
            //Imports System.Math 
            //namespace appbox.Reporting.vbgen
            ...
            lines.Add("Imports appbox.Reporting.RDL");
            lines.Add("namespace appbox.Reporting.vbgen");
...
//						tsw.Close();
            ...
            if (RdlEngineConfig.DirectoryLoadedFrom == null) {
                if (File.Exists(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "appbox.Reporting.dll"))) {
                    re = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "appbox.Reporting.dll");   // this can fail especially in web scenarios
                }
                else
                {
                    re = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Bin");   // this can work especially in web scenarios
                    re = Path.Combine(re, "appbox.Reporting.dll");  
                }
            }
            else
                re = Path.Combine(RdlEngineConfig.DirectoryLoadedFrom, "appbox.Reporting.dll");     // use RdlEngineConfig.xml directory when available
            ...
            if (this.OwnerReport.CodeModules != null)
            {
                ...
                {
                    //Changed from Forum, User: solidstate
		...
		internal Type CodeType()
		{
			...
			try
			{
				object instance = _Assembly.CreateInstance("appbox.Reporting.vbgen." + this._Classname, false); 
				t = instance.GetType();
			}
			catch (Exception e)
			{
		...
		internal object Load(Report rpt)
		{
			...
			try
			{
                ...
                args[0] = rpt;
				wc.Instance = _Assembly.CreateInstance("appbox.Reporting.vbgen." + this._Classname, false, BindingFlags.CreateInstance, null, args, null, null); 
			}
			catch (Exception e)
			{
				wc.Instance = null;
